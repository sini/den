...
sequenceDiagram
    participant root as deploy
    participant user_to_default as user-to-default

    root ->> user_to_default: dispatch
    activate user_to_default
    Note over user_to_default: default/resolve(default), provides/default/resolve(define-user):den/provides(host, user), default/resolve(deploy,server)(host), provides/default/resolve(hostname):den/provides(host)<br/>provides/default/resolve(mutual-provider):den/provides(host, user), provides/define-user, provides/hostname, provides/mutual-provider
