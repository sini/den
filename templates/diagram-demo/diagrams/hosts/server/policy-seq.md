...
sequenceDiagram
    ...
    Note over host_to_default: default/resolve(default), provides/default/resolve(define-user):den/provides, provides/default/resolve(hostname):den/provides(host), provides/default/resolve(mutual-provider):den/provides<br/>default/resolve(server)(host), provides/define-user, provides/hostname, provides/mutual-provider
    ...
    Note over host_to_hm_host: hm-host/hm-host(host), hm-host/resolve(hm-host)(host, user), provides/hm-host/resolve(hm-host):den/provides(host, user), hm-host/resolve(server)<br/>hm-user
    ...
    Note over host_to_users: deploy(host, user)
    Note over host_to_users: user/resolve(deploy,server), user/resolve(user)(host, user)
