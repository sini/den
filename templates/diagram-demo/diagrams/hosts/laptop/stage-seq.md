...
sequenceDiagram
    ...
    host ->> host: laptop(host)
    deactivate host
    Note over host: default, desktop, hm-host, host/resolve(<anon>:0)<br/>networking, virtualization/podman, regreet, tailscale<br/>user, virtualization, workstation

    activate p_default
    p_default ->> p_default: provides/default/resolve(hostname):den/provides(host)
    p_default ->> p_default: default/resolve(laptop)(host)
    ...
    deactivate hm_host
    Note over hm_host: hm-host/resolve(laptop), hm-user, alice/to-hosts
    ...
    deactivate user
    Note over user: demo-shell, dev-tools, hyprland, user/resolve(alice)<br/>user/resolve(alice,laptop)
```
