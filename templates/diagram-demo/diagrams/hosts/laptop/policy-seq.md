...
sequenceDiagram
    ...
    Note over host_to_default: default/resolve(default), provides/default/resolve(define-user):den/provides, provides/default/resolve(hostname):den/provides(host), default/resolve(laptop)(host)<br/>provides/default/resolve(mutual-provider):den/provides, provides/define-user, provides/hostname, provides/mutual-provider
    ...
    Note over host_to_hm_host: hm-host/hm-host(host), hm-host/resolve(hm-host)(host, user), provides/hm-host/resolve(hm-host):den/provides(host, user), hm-host/resolve(laptop)<br/>hm-user, alice/to-hosts
    ...
    host_to_users ->> host_to_users: provides/primary-user(host, user)
    host_to_users ->> host_to_users: user/resolve(alice)
    Note over host_to_users: provides/unfree(nvidia-x11,nvidia-settings)(class), provides/unfree(vscode)(class), user/resolve(alice,laptop), user/resolve(user)(host, user)
