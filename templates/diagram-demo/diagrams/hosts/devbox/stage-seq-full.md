# Stage Sequence (expanded): devbox
...
sequenceDiagram
    ...
    deactivate host
    Note over host: default, desktop, gdm, hm-host<br/>monitoring, monitoring/alerting, monitoring/nginx-exporter, monitoring/node-exporter<br/>networking, policy:host-to-default, policy:host-to-hjem-host, policy:host-to-hm-host<br/>policy:host-to-maid-host, policy:host-to-users, policy:host-to-wsl-host, provides/primary-user<br/>regreet, server, tailscale, user<br/>virtualization, virtualization/docker, virtualization/podman, workstation
    ...
    deactivate hm_host
    Note over hm_host: alice/to-hosts, hm-user, policy:hm-host-to-hm-user

    hm_host -->> host: forward: alice/to-hosts
    ...
    Note over user: ── user
    activate user
    user ->> user: alice(host, user)
    user ->> user: bob(host, user)
    user ->> user: provides/unfree(nvidia-x11,nvidia-settings)(class)
    user ->> user: provides/unfree(vscode)(class)
    deactivate user
    Note over user: demo-shell, dev-tools, gnome, hyprland<br/>policy:user-to-default
