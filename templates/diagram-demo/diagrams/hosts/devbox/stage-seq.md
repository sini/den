...
sequenceDiagram
    ...
    host ->> host: devbox(host)
    host ->> host: host/resolve(host)(host)
    deactivate host
    Note over host: monitoring/alerting, default, desktop, virtualization/docker<br/>gdm, hm-host, host/resolve(<anon>:0), monitoring<br/>networking, monitoring/nginx-exporter, monitoring/node-exporter, virtualization/podman<br/>provides/primary-user, regreet, server, tailscale<br/>user, virtualization, workstation

    activate p_default
    p_default ->> p_default: default/resolve(devbox)(host)
    ...
    deactivate hm_host
    Note over hm_host: hm-host/resolve(alice,devbox), hm-host/resolve(devbox), hm-user, alice/to-hosts
    ...
    deactivate user
    Note over user: demo-shell, dev-tools, gnome, hyprland<br/>user/resolve(alice), user/resolve(alice,devbox), user/resolve(bob,devbox)
```
