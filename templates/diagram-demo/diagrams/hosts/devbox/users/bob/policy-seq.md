...
sequenceDiagram
    participant root as bob
    participant user_to_default as user-to-default

    root ->> user_to_default: dispatch
    activate user_to_default
    Note over user_to_default: default/resolve(bob,devbox)(host), default/resolve(default), provides/default/resolve(define-user):den/provides(host, user), provides/default/resolve(hostname):den/provides(host)<br/>provides/default/resolve(mutual-provider):den/provides(host, user), provides/define-user, provides/hostname, provides/mutual-provider
