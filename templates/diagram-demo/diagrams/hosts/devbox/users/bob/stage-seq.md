...
sequenceDiagram
    ...
    deactivate user
    Note over user: default, dev-tools, gnome, user/resolve(<anon>:0)<br/>user/resolve(<anon>:1)

    activate p_default
    p_default ->> p_default: default/resolve(bob,devbox)(host)
    ...
    Note over p_default: default/resolve(default), provides/define-user, provides/hostname, provides/mutual-provider
```
