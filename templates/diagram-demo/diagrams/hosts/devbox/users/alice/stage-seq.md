...
sequenceDiagram
    ...
    user ->> user: provides/primary-user(host, user)
    ...
    deactivate user
    Note over user: default, demo-shell, dev-tools, hyprland<br/>user/resolve(<anon>:0), user/resolve(<anon>:1), user/resolve(alice)

    activate p_default
    p_default ->> p_default: default/resolve(alice,devbox)(host)
    p_default ->> p_default: provides/default/resolve(define-user):den/provides(host, user)
    p_default ->> p_default: provides/default/resolve(hostname):den/provides(host)
    p_default ->> p_default: provides/default/resolve(mutual-provider):den/provides(host, user)
    deactivate p_default
    Note over p_default: default/resolve(default), provides/define-user, provides/hostname, provides/mutual-provider<br/>alice/to-hosts
```
