# Resolution Sequence (expanded): devbox

![Sequence expanded](./devbox-seq-full.mmd.svg)
...
sequenceDiagram
    ...
    Note over host: backup, desktop, monitoring, monitoring/alerting, monitoring/nginx-exporter, monitoring/node-exporter, networking, regreet, server, tailscale, virtualization, virtualization/docker, virtualization/podman, workstation

    host ->> p_default: resolve
    host ->> hm_host: resolve
    host ->> user: resolve

    Note over p_default: ── default { host }
    Note over p_default: alice/to-hosts, default, provides/define-user, provides/hostname, provides/mutual-provider

    p_default -->> host: forward: alice/to-hosts

    Note over hm_host: ── hm-host { host }
    Note over hm_host: hm-host

    hm_host ->> hm_user: resolve

    Note over hm_user: ── hm-user { host, user }
    Note over hm_user: hm-user

    Note over user: ── user { host, user }
    Note over user: alice, demo-shell, dev-tools, hyprland, primary-user, provides/primary-user, user
