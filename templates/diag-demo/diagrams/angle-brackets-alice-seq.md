# Resolution Sequence: angle-brackets-alice

![Sequence](./angle-brackets-alice-seq.mmd.svg)
...
sequenceDiagram
    ...
    participant p_default as default { host, user }

    user ->> p_default: resolve

    Note over user: alice, demo-shell, dev-tools, hyprland, primary-user, provides/primary-user, +3 more
    Note over p_default: default, default/aspect(default), provides/default/aspect(default):den/provides, default/cross-provide(user), default/self-provide(default), provides/define-user, +3 more
```
