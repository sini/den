...
in
{
  perSystem =
    ...
    let
      ...
      theme = diag.themeFromBase16 { inherit pkgs; scheme = themeScheme; };
...
'';
      ...
      aspectsSourcesByHost = lib.listToAttrs (map (host: {
        name = host.name;
...
'';
      ...
      inherit (diag.export) entityEntries entriesToPackages entriesToFiles entryCopyLine;
    ...
    in
    {
      packages = allPackages // {
        write-diagrams = pkgs.writeShellScriptBin "write-diagrams" ''
          set -euo pipefail
          dest="$(${pkgs.git}/bin/git rev-parse --show-toplevel)/templates/diag-demo"
