...
[templates/microvm](https://github.com/denful/den/tree/main/templates/microvm) demonstrates Den's extensibility: custom `den.stages` and `den.schema` extensions for integrating other Nix libraries like [MicroVM.nix](https://github.com/microvm-nix/microvm.nix).
...
den.stages.microvm-host.provides.microvm-host = { host }: {
...
den.stages.microvm-host.provides.microvm-guest = { host, vm }: {
  includes = [
