...
[`templates/minimal`](https://github.com/denful/den/tree/main/templates/minimal)
</Aside>
...
3. **User aspect** `den.aspects.tux` — includes the [`primary-user`](/reference/batteries/#den-batteries-primary-user) battery

### How It Works

```mermaid
graph TD
  H["den.hosts.x86_64-linux.igloo"] -->|"host scope"| A["den.aspects.igloo"]
  H -->|"user scope"| U["den.aspects.tux"]
  U -->|"includes"| PU["den.batteries.primary-user"]
  A -->|"resolve nixos"| NixOS["nixosConfigurations.igloo"]
```

1. Den reads `den.hosts` and creates a **host scope** for `igloo`
2. The host aspect `igloo` is activated with its NixOS config
3. For each user, a **user scope** (carrying both host and user) is created
