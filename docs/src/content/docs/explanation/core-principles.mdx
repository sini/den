...
**Aspects** and **Contexts** are Den core architectural principles.

It is how Den supports different platform configurations, packages/checks flake outputs, different home environments support,
...
aspect = den.stages.host { host = den.hosts.x86_64-linux.my-laptop; };
...
When Den docs say "context" they mean the pipeline data shape (`{ host }`, `{ host, user }`,
`{ home }`) — real Nix function arguments. This is distinct from NixOS module arguments
(`{ config, pkgs, lib, ... }`) which live inside the module system. Den context is
evaluated _before_ module evaluation, which is why it cannot cause infinite recursion.
</Aside>


### Den is Context-driven

Den uses `den.stages.<name>.provides.<name>` as **Aspect Pointcuts** where configuration is applied to data at that point in the pipeline.
Say you have an entity kind `foo` with data shape `{ x }`:

```nix
# This policy fans out from entity kind `foo` to entity kind `bar`
den.policies.foo-to-bar = { x }: [ { y = x; } ]
```

The following `den.stages.bar.provides.bar` is the actual pointcut that **locates** the aspect responsible for configuring
using data `{ y }` available when resolving `bar` entities.

```nix
# inlined aspect for this example, but can locate the aspect by any means
den.stages.bar.provides.bar = { y }: { nixos.something = y; }
```

and since `den.stages.bar` **is** itself an aspect, it can be used to include
additional configurations at that particular stage:

```nix
den.stages.bar.nixos.something-else = true;
den.stages.bar.includes = [ den.aspects.other ];
```

### `den.stages.host` pipeline

This is how everything works in Den, the following is the stage transitions
that happen when applying `den.stages.host { host }`

```nix
# host configuration: THIS is where your host-aspect is hooked into the pipeline
den.stages.host.provides.host = { host }: den.aspects.${host.aspect};

# host -> users stage transition (the `host-to-users` policy)
den.policies.host-to-users = { host }: map (user: { inherit host user; }) (lib.attrValues host.users); # transition into many { host, user }

# user configuration: Lookup user aspect.
den.stages.user.provides.user = { host, user }: den.aspects.${user.aspect}; # Hook for the user-aspect.

# conditional transition ONLY if hm is enabled for user and host.
den.policies.user-to-hm-user = { host, user }: lib.optional (host.hm.enable && lib.elem "homeManager" user.classes) { inherit host user; }

# host -> wsl-host: Same data shape, different stage.
den.policies.host-to-wsl-host = { host }: lib.optional host.wsl.enable { inherit host; }
```
...
Minimal real-life example: [wsl support](https://github.com/denful/den/blob/main/modules/aspects/provides/wsl.nix)
...
[flake-parts perSystem classes](https://github.com/denful/den/blob/main/templates/flake-parts-modules/modules/custom-classes.nix)
...
| Stages | Transform data flow | `den.stages.host` produces `{host}`, then `{host, user}` per user |
| Policies | Entity topology | `den.policies.host-to-users` fans out from hosts to users |
