...
Den uses `den.stages.<name>.provides.<name>` as **Aspect Pointcuts** where configuration is applied to data in that stage.
Say you have a data shape: `{ x }`, that you name as a `foo` stage:

```nix
# This says how to move from one stage `foo` into another stage `bar`
...
using data `{ y }` available at `bar` stage.
