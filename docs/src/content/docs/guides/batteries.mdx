...
[`modules/aspects/provides/`](https://github.com/denful/den/tree/main/modules/aspects/provides)
</Aside>
...
```nix
# Import per host
den.stages.host.includes = [ (den.provides.import-tree._.host ./hosts) ];

# Import per user
den.stages.user.includes = [ (den.provides.import-tree._.user ./users) ];
...
```nix
den.aspects.laptop.includes = [
  (den.provides.insecure [ "foo-1.2.3" ])
];
```
