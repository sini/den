...
<CardGrid>
  ...
  <Card title="Resolution Pipeline" icon="right-arrow">
    Declarative [stages](/reference/ctx/) with `den.stages`.

    [Policies](/explanation/policies/) drive traversal between stages,
    using a declarative design, ensuring only valid contexts exist via
...
```nix
# Use Den API -- Resolution pipeline runs here, nothing is configured yet.
aspect = den.stages.host { host = den.hosts.x86_64-linux.igloo; };
...
3. **Resolution pipeline** -- `den.stages` transforms schema entries into stage pairs (`{host}`, `{host, user}`, `{home}`), walking policy transitions for derived stages like `hm-host`, `hm-user`, `wsl-host` or any other custom stage.
