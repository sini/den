  ...
  pushScopeHandler = {
    "push-scope" =
      ...
      let
        ...
        updatedContexts = prevContexts // { ${newScopeId} = scopedCtx; };
        updatedParent = prevParent // lib.optionalAttrs (!isSameScope) { ${newScopeId} = parentScope; };
        updatedPolicies = prevPolicies // { ${newScopeId} = prevPolicies.${newScopeId} or { }; };
        updatedEntityClass = prevEntityClass // lib.optionalAttrs (entityClass != null) { ${newScopeId} = entityClass; };
        updatedEntityKind = prevEntityKind // lib.optionalAttrs (entityKind != null) { ${newScopeId} = entityKind; };
        updatedSourcePolicy = prevSourcePolicy // lib.optionalAttrs (sourcePolicyName != null) { ${newScopeId} = sourcePolicyName; };
      in
      {
        resume = {
