...
let
  ...
  # --- C4 Component: aspects grouped by stage inside one host ---
  #
  ...
  toC4ComponentWith =
    ...
    let
      ...
      # Only render edges where both endpoints are kept (meaningful aspects).
  ...
  # --- C4 Container: one box per ctx stage / class, no components inside ---
  #
  ...
  # --- C4 Context: fleet-wide overview ---
  #
  ...
  # Body-builder for the component view, shared between plantuml and
  # mermaid. Takes the same graph and returns a list of body lines
  # (without the frame wrapper).
  c4ComponentBody =
    theme: graph:
    let
      inherit (graph)
